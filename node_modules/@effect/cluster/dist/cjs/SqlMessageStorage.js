...
`.pipe(Effect.tapDefect(error => Effect.annotateLogs(Effect.logDebug("Failed to create indexes", error), {
  ...
  return yield* MessageStorage.makeEncoded({
    ...
    requestIdForPrimaryKey: primaryKey => sql`SELECT id FROM ${messagesTableSql} WHERE message_id = ${primaryKey}`.pipe(Effect.map(rows => Option.fromNullable(rows[0]?.id).pipe(Option.map(Snowflake.Snowflake))), Effect.provideService(SqlClient.SafeIntegers, true), _ClusterError.PersistenceError.refail),
